...
//	    }
	...
	for (;;) {
		/*
		 * Receive packets on a port and forward them to pofswitch
		 */
		for (port = 0; port < nb_ports; port++) {
...
//    for(int i = 0; i < MAX_DPDK_PORT_NUM; ++i) {
//        dpdk_port[i].port_mac_addr = (struct ether_addr *)malloc(TWPETH);
//    }

	get_dpdk_port_info(dpdk_port);
    printf("here: \n");
//    printf("addr of %p\n", dpdk_port[1].port_mac_addr->addr_bytes);
//    POF_COMMAND_PRINT_0X_NO_ENTER(rte_eth_devices[1].data->mac_addrs, POF_ETH_ALEN);
   // POF_COMMAND_PRINT_0X_NO_ENTER(dpdk_port[1].port_mac_addr, POF_ETH_ALEN);
	for (port = 0; port < nb_ports; port++) {
